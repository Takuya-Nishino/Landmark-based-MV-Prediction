...
- KDE lines are removed for lighter and cleaner manuscript figure
- Panel labels are integrated into titles
...
# =========================================================
def parse_args():
    parser = argparse.ArgumentParser(description="Create TIFF and JPEG event timing figures for LM0 and LM3.")
    parser.add_argument("--excel-path", required=True, help="Path to source Excel file")
    parser.add_argument("--out-dir", required=True, help="Output directory")
    parser.add_argument(
        "--out-file-base",
        default="Figure_Event_Timing_LM0_LM3_commonriskset",
...
COLOR_TRACH = "steelblue"
COLOR_DEATH = "sandybrown"
...
def save_jpeg_from_figure(fig: plt.Figure, out_path: str, dpi: int = JPEG_DPI, quality: int = JPEG_QUALITY) -> None:
    tmp_png = out_path.replace(".jpg", ".tmp.png")
    fig.savefig(tmp_png, dpi=dpi, format="png", bbox_inches="tight", facecolor="white")
...
def plot_panel(
    ...
    trach_label = build_legend_label("Trach", trach_days, denom)
    ...
    ax.set_xlabel("Days after landmark", fontsize=FS_AXIS)
    ax.set_ylabel("Number of events", fontsize=FS_AXIS)
    ax.set_title(f"{panel_label}  {title}", fontsize=FS_TITLE, fontweight="bold", pad=10)
    ax.grid(False)
...
outpath_tiff = os.path.join(OUT_DIR, OUT_FILE_BASE + ".tiff")
outpath_jpg = os.path.join(OUT_DIR, OUT_FILE_BASE + ".jpg")
...
save_jpeg_from_figure(
    fig=fig,
    out_path=outpath_jpg,
    dpi=JPEG_DPI,
...
print(f"Saved TIFF: {outpath_tiff}")
print(f"Saved JPEG: {outpath_jpg}")
